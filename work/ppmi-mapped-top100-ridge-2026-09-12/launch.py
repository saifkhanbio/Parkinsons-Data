"""Fixed top-100 mapped-gene ridge classifier."""
...
from top100 import (BASE, PRIOR, HALLMARK, MAPPED, SELECTION, MappedData, choose_device, make_splits,
    SEED, CS, KS, fit_model, tune, rank, select, artifact, predict_artifact, evaluate, validate)
...
def prepare():
    ...
    selection_hashes=json.loads((SELECTION/'input_sha256.json').read_text())
    assert json.loads((SELECTION/'summary.json').read_text())['status']=='COMPLETE'
    for p,expected in selection_hashes.items():
        assert sha(p)==expected,p
    paths = [Path(p) for p in old]+[Path(p) for p in selection_hashes]
    paths += [SELECTION/'inner_tuning_scores.tsv',SELECTION/'input_sha256.json',HALLMARK/'hallmark_source.tsv']
    ...
    paths += [ROOT / n for n in ['README.md', 'top100.py', 'launch.py', 'predict.py', 'candidate_genes.tsv', 'fold_plan.json']]
...
def summarize(pred, fold_metrics):
    selected = pd.DataFrame(fold_metrics).assign(variant='top100_ridge')
    old = pd.read_csv(MAPPED/'outer_fold_metrics.tsv', sep='\t')
    old = old[old.penalty=='ridge'].assign(variant='original_ridge')
    paired = selected.merge(old, on=KEYS, suffixes=('_selected', '_original'), validate='one_to_one')
    assert len(paired)==30
    for name in ['n_train','n_test','eligible_genes']:
        assert np.array_equal(paired[name+'_selected'],paired[name+'_original'])
    paired['AUROC_difference'] = paired.test_AUROC_selected-paired.test_AUROC_original
    ...
    combined = pd.concat([selected,old],ignore_index=True)
...
def train():
    ...
    previous_top100=pd.read_csv(SELECTION/'inner_tuning_scores.tsv',sep='\t',dtype={'k':str})
    previous_top100=previous_top100[previous_top100.k=='100'].set_index(KEYS+['C'])
    predictions, metrics, params, tuning, coefficients, checks, eligible_rows = [], [], [], [], [], [], []
    for number, split in enumerate(plan,1):
        ...
        for row in rows:
            if row['k']=='100':
                reference=previous_top100.loc[key+(row['C'],)]
                np.testing.assert_allclose(row['mean_inner_AUROC'],reference.mean_inner_AUROC,atol=1e-12,rtol=0)
                np.testing.assert_allclose(np.fromstring(row['inner_AUROCs'],sep=';'),np.fromstring(reference.inner_AUROCs,sep=';'),atol=1e-12,rtol=0)
        ...
        chosen = best['top100_ridge']
        order, f_scores = rank(pack,y[tr])
        selected, local = select(pack,order,chosen['k'])
        assert chosen['k']=='100' and selected['train'].shape[1]==100
        ...
        assert set(state['selected_gene_ids']) <= set(genes[pack['state']['indices']])
        eligible_rows.extend(dict(**context,Geneid=g) for g in genes[pack['state']['indices']])
        ...
        metrics.append(dict(**context,n_train=len(tr),n_test=len(te),eligible_genes=pack['train'].shape[1],
            selected_genes=selected['train'].shape[1],k=chosen['k'],
            train_AUROC=float(roc_auc_score(y[tr],model.predict_proba(selected['train'])[:,1])),
            test_AUROC=float(roc_auc_score(y[te],p))))
        params.append(dict(**context,**chosen,eligible_genes=pack['train'].shape[1],selected_genes=selected['train'].shape[1],
                           optimizer_iterations=int(model.n_iter_[0])))
        ranks=np.empty(len(order),dtype=int); ranks[order]=np.arange(1,len(order)+1)
        coefficients.extend(dict(**context,Geneid=g,coefficient=float(c),ANOVA_F=float(f_scores[j]),training_rank=int(ranks[j]))
                            for g,c,j in zip(state['selected_gene_ids'],model.coef_[0],local))
    ...
    assert len(coefficients)==3000
    table('outer_gene_coefficients.tsv',coefficients)
    table('outer_eligible_genes.tsv',eligible_rows)
    selected_counts=pd.DataFrame(coefficients).groupby(['protocol','Geneid']).size().rename('selected_fits')
    eligible_counts=pd.DataFrame(eligible_rows).groupby(['protocol','Geneid']).size().rename('eligible_fits')
    retention=pd.concat([eligible_counts,selected_counts],axis=1).fillna(0).astype(int).reset_index()
    retention['total_fits']=15
    assert (retention.selected_fits<=retention.eligible_fits).all()
    table('gene_selection_frequency.tsv',retention)
    status('SUMMARIZING')
    ...
    order, f_scores = rank(full,y)
    selected, local = select(full,order,chosen['k'])
    assert chosen['k']=='100' and selected['train'].shape[1]==100
    model = fit_model(selected['train'],y,chosen['C'],'ridge',0.)
    state = artifact(model,selected,chosen,genes)
    ...
    path = ROOT/'models'/'mapped_top100_ridge.joblib'
    joblib.dump(state,path)
    np.testing.assert_allclose(predict_artifact(joblib.load(path),raw[:5]),model.predict_proba(selected['train'][:5])[:,1],atol=1e-9)
    save('final_parameters.json',chosen)
    table('final_selected_genes.tsv',dict(Geneid=state['selected_gene_ids'],coefficient=model.coef_[0],ANOVA_F=f_scores[local]))
    all_ranks=np.empty(len(order),dtype=int); all_ranks[order]=np.arange(1,len(order)+1)
    panel=pd.DataFrame(dict(rank=all_ranks[local],Geneid=state['selected_gene_ids'],
        coefficient=model.coef_[0],ANOVA_F=f_scores[local])).sort_values('rank')
    annotations=pd.read_csv(HALLMARK/'hallmark_source.tsv',sep='\t')
    symbols=annotations.groupby('ensembl_gene').gene_symbol.agg(lambda x:';'.join(sorted(set(x.dropna().astype(str)))))
    panel['gene_symbol']=panel.Geneid.str.split('.').str[0].map(symbols)
    assert len(panel)==100 and panel.Geneid.is_unique and panel['rank'].tolist()==list(range(1,101))
    table('top100_gene_panel.tsv',panel)
    finalize(summary,full['train'].shape[1],selected['train'].shape[1])


def finalize(summary,eligible_count,selected_count):
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    repeats=pd.read_csv(ROOT/'primary_per_repeat.tsv',sep='\t')
    params=pd.read_csv(ROOT/'selected_parameters.tsv',sep='\t',dtype={'k':str})
    table('selected_parameter_frequency.tsv',params.groupby(['protocol','k','C']).size().rename('folds').reset_index())
    fig,axes=plt.subplots(1,2,figsize=(9,4),sharey=True)
    for ax,protocol in zip(axes,['participant_stratified','batch_grouped']):
        for repeat in sorted(repeats.repeat.unique()):
            values=repeats[(repeats.protocol==protocol)&(repeats.repeat==repeat)].set_index('variant').loc[['original_ridge','top100_ridge'],'AUROC']
            ax.plot([0,1],values,marker='o',label=f'Repeat {repeat}')
        ax.axhline(.5,color='gray',ls='--')
        ax.set(xticks=[0,1],xticklabels=['Original ridge','Top-100 ridge'],ylim=(0,1),title=protocol)
    axes[0].set_ylabel('Mean within-fold AUROC per repeat'); axes[1].legend(fontsize=8)
    fig.tight_layout(); fig.savefig(ROOT/'top100_ridge_comparison.png',dpi=170); plt.close(fig)
    hashes=json.loads((ROOT/'input_sha256.json').read_text())
    assert all(sha(p)==v for p,v in hashes.items())
    lines=['# Fixed top-100 mapped-gene ridge classifier','',
        '528 participants, fixed 4,376 mapped candidates and the same 30 nested outer folds. Exactly 100 genes are selected by ANOVA within each training partition; only C=.001,.01,.1,1 is tuned. The all-genes ridge is a separate benchmark, never a candidate for this fixed-100 model.', '',
    ...
    lines += ['', 'Primary AUROC averages within-fold AUROCs, then three repeat means. Repeat ranges are descriptive, not confidence intervals. Paired selection-minus-original differences are in paired_summary.tsv. The new score evaluates the fixed top-100 training-selection procedure. Each outer fold can select different genes; this does not independently validate the one final full-data gene panel.', '',
        'Original all-genes ridge inner scores, selected C/threshold and outer predictions reproduced in all 30 folds, as did prior top-100 inner candidate scores. Every fitted top-100 model retained exactly 100 genes. Count/cohort integrity, exact eligible genes and participants, nested boundaries, CPU/RTX preprocessing, F versus t-squared, deterministic ties, synthetic ranking, held-out perturbation, convergence, serialized inference and source hashes passed.', '',
        'One full-data 100-gene development artifact and its ranked, annotated gene panel are saved in models/mapped_top100_ridge.joblib and top100_gene_panel.tsv. Outer selected-gene frequencies are descriptive; they were not reused to define the model. The full raw gene universe remains necessary for library normalization at prediction time.', '',
        'This is an adaptive exploratory comparison on a repeatedly examined cohort. Association-selected genes can reflect cell composition or technical effects and are not established disease-specific markers. Independent-cohort validation remains outstanding.', '',
        '![Comparison](top100_ridge_comparison.png)','']
    (ROOT/'RESULTS.md').write_text('\n'.join(lines))
    save('summary.json',dict(status='COMPLETE',participants=528,candidate_genes=4376,final_eligible_genes=eligible_count,
        final_selected_genes=selected_count,primary_results=summary.to_dict(orient='records'),validation='PASS',
        benchmark_reproduction_folds=30,source_hashes_unchanged=True,external_validation=False))
