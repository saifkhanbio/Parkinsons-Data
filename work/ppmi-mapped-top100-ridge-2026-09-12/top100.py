"""Fixed top-100 ANOVA ridge with a separate all-genes control."""
...
CS=[.001,.01,.1,1.]
KS=['100','all']
SELECTION=MAPPED.parent/'ppmi-mapped-gene-selection-2026-09-12'
...
def tune(data,train,splits,ks=None,cs=None):
    ...
    best={}
    for variant,candidates in [('original_ridge',[r for r in rows if r['k']=='all']),('top100_ridge',[r for r in rows if r['k']=='100'])]:
...
def validate(device):
    ...
    best,rows=tune(data,tr,splits,ks=['100','all'],cs=[.001,.1])
    ...
    other,other_rows=tune(changed,tr,splits,ks=['100','all'],cs=[.001,.1])
    assert best==other and rows==other_rows
    np.testing.assert_array_equal(order,rank(changed.pack(tr,te),y[tr])[0])
    assert best['top100_ridge']['k']=='100'
    chosen=best['top100_ridge']; selected,_=select(pack,order,chosen['k'])
    assert selected['train'].shape[1]==100
