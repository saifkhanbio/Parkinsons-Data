"""Run the fixed Step 2 nested comparison; all writes stay beside this script."""
...
from demographic_models import (BASE,PRIOR,HALLMARK,MAPPED,FAMILIES,CS,DemographicData,
...
def prepare(device):
    ...
    inputs += [BASE/"out_of_fold_predictions.tsv"]
    ...
    inputs += [HERE/n for n in ["README.md","demographic_models.py","launch.py","reporting.py","predict.py"]]
...
def train(device):
    ...
    initial=pd.read_csv(BASE/"out_of_fold_predictions.tsv",sep="\t",dtype={"PATNO":str}).query("family=='demographic'")
    ...
    for number,split in enumerate(plan,1):
        ...
        for (family,c),p in inner_p.items():inner_saved[f"{number}_{family}_{c}"]=p
        ...
        for family in FAMILIES:
            x,v=matrices(pack,family);chosen=best[family]
            ...
            if family=="demographic" and split["repeat"]==1:
                prior_demo=initial[(initial.protocol==split["protocol"])&(initial.outer_fold==split["fold"])].set_index("PATNO").loc[meta.PATNO.iloc[te]]
                np.testing.assert_allclose(p,prior_demo.probability_PD,atol=1e-8,rtol=1e-8)
                checks.append(dict(**context,family=family,max_probability_difference=float(np.max(abs(p-prior_demo.probability_PD))),status="PASS"))
            train_p=model.predict_proba(x)[:,1]
            metrics.append(dict(**context,family=family,n_train=len(tr),n_test=len(te),eligible_genes=0 if family=="demographic" else pack["train"].shape[1],
            ...
            np.testing.assert_allclose(predict_artifact(state,raw[te[:5]],meta.iloc[te[:5]]),p[:5],atol=1e-9,rtol=1e-9)
            features=([] if family=="rna" else ["age_collection_years","male"])+([] if family=="demographic" else genes[pack["state"]["indices"]].tolist())
            coefficients.extend(dict(**context,family=family,feature=f,coefficient=float(c)) for f,c in zip(features,model.coef_[0]))
    ...
    for family in FAMILIES:
        chosen=best[family];x,_=matrices(pack,family)
    ...
    result.update(status="COMPLETE",participants=528,source_hashes_unchanged=True,validation="PASS",
                  reproduced_RNA_folds=30,reproduced_initial_demographic_folds=10,
                  elapsed_seconds=time.monotonic()-started,device=str(device),main_model_fits=1233)
...
if __name__=="__main__":
    ...
    try:
        with threadpool_limits(limits=2):
            ...
            if "--require-gpu" in sys.argv and device.type!="cuda":raise RuntimeError("Requested CUDA/RTX is unavailable")
