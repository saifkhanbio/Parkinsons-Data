...
from blood_models import FAMILIES,evaluate
...
def report(root,meta,pred,fold_metrics,device):
    ...
    for pi,protocol in enumerate(PROTOCOLS):
        ...
        for family,reference_family in [("rna_blood","rna"),("rna_blood","blood"),("blood","rna")]:
        ...
        for group in masks:
            subgroup_delta.append(add_interval(dict(protocol=protocol,subgroup=group,comparison="rna_blood minus rna"),model_subgroups[("rna_blood",group)]-model_subgroups[("rna",group)]))
    ...
    paired=[]
    for family,ref in [("rna_blood","rna"),("rna_blood","blood")]:
    ...
    labels={"rna":"RNA-only ridge","blood":"Measured-cell ridge","rna_blood":"RNA + measured cells"}
    lines=["# Hallmark RNA plus measured blood cells with separate penalties", "",
        "Same 528 participants and all original nested folds. RNA preprocessing and the unweighted RNA-only benchmark remain fixed. Five measured blood-cell variables are added; RNA and blood penalties are selected independently within inner folds.","",
        "## Primary comparison","",
    ...
    lines += ["","## Demographic reporting","",
        "Subgroup tables use mean repeat-specific pooled out-of-fold metrics, as in Step 1. This differs from the primary within-fold estimand above. Counts and prevalence are in subgroup_counts.tsv; matched RNA+blood-minus-RNA changes and conditional intervals are in subgroup_paired_comparisons.tsv.","",
        "## Validation and interpretation","",
        "The original RNA-only inner AUROCs, selected regularization, gene identities, thresholds and predictions reproduce in all 30 folds. All model fits converged; all artifacts passed inference roundtrips. Preflight excludes held-out influence on filtering, scaling, tuning and thresholds.","",
        ...
        "No class weighting, age/sex predictors, separate-sex model, random forest or DE screening is included. The blood-only model uses five measured-cell predictors; it is distinct from the earlier seven-variable blood_clinical baseline. CBC measurements may precede RNA collection by up to two months. Predictive use of blood counts does not establish cell-independent expression. The maximum observed score across sequential analyses is not an unbiased estimate of an algorithm-selection procedure. No change in clinical utility or disability outcomes is established.","",
        "![Model comparison](hallmark_blood_comparison.png)","",
...
def make_figure(root,primary,repeats):
    ...
    for col,protocol in enumerate(PROTOCOLS):
        ...
        for row,metric in enumerate(["AUROC","AP"]):
            ...
            ax.set_xticks(range(3),["RNA only","Blood cells","RNA +\nblood cells"])
            ax.set_xlim(-.4,2.4)
    ...
    fig.suptitle("Adding measured blood cells to Hallmark RNA ridge",fontsize=15,y=.97)
    ...
    for ext in ["png","pdf","svg"]:fig.savefig(root/f"hallmark_blood_comparison.{ext}",dpi=600,facecolor="white")
    fig.savefig(root/"hallmark_blood_comparison_preview.png",dpi=120,facecolor="white")
    plt.close(fig)
