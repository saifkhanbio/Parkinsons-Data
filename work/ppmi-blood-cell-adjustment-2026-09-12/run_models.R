...
stopifnot(nrow(m)==528, !anyDuplicated(m$PATNO), nrow(primary)==21888, all(m$PATNO %in% colnames(primary)))
...
stopifnot(identical(as.character(d$group),m$group),sum(d$group=="PD")==358,sum(d$group=="Control")==170)
