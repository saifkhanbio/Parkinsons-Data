"""Controlled mapped-gene ranking plus ridge."""
...
from gene_selection import (BASE, PRIOR, HALLMARK, MAPPED, MappedData, choose_device, make_splits,
...
def prepare():
    ...
    assert pack['train'].shape[1]>=2000
    ...
    paths += [ROOT / n for n in ['README.md', 'gene_selection.py', 'launch.py', 'predict.py', 'candidate_genes.tsv', 'fold_plan.json']]
...
def summarize(pred, fold_metrics):
    selected = pd.DataFrame(fold_metrics).assign(variant='selected_ridge')
...
def train():
    ...
    path = ROOT/'models'/'mapped_selected_ridge.joblib'
...
def finalize(summary,eligible_count,selected_count):
    ...
    for ax,protocol in zip(axes,['participant_stratified','batch_grouped']):
        for repeat in sorted(repeats.repeat.unique()):
            values=repeats[(repeats.protocol==protocol)&(repeats.repeat==repeat)].set_index('variant').loc[['original_ridge','selected_ridge'],'AUROC']
            ax.plot([0,1],values,marker='o',label=f'Repeat {repeat}')
        ax.axhline(.5,color='gray',ls='--')
        ax.set(xticks=[0,1],xticklabels=['Original ridge','Selection + ridge'],ylim=(0,1),title=protocol)
    axes[0].set_ylabel('Mean within-fold AUROC per repeat'); axes[1].legend(fontsize=8)
    fig.tight_layout(); fig.savefig(ROOT/'gene_selection_comparison.png',dpi=170); plt.close(fig)
    hashes=json.loads((ROOT/'input_sha256.json').read_text())
    assert all(sha(p)==v for p,v in hashes.items())
    lines=['# Controlled gene selection plus ridge','',
        '528 participants, fixed 4,376 mapped candidates and the same 30 nested outer folds. Training-only ANOVA ranking with k=100,500,1000,2000,all is jointly tuned with C=.001,.01,.1,1. All eligible genes remain an option in the new selection procedure.', '',
    ...
    lines += ['', 'Primary AUROC averages within-fold AUROCs, then three repeat means. Repeat ranges are descriptive, not confidence intervals. Paired selection-minus-original differences are in paired_summary.tsv. The new score evaluates inner-fold selection of both k and C, not a hindsight choice of k from outer results.', '',
        ...
        '![Comparison](gene_selection_comparison.png)','']
