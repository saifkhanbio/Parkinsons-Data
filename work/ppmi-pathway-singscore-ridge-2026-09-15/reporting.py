...
from path_models import FAMILIES,PATH_FAMILIES,evaluate
...
def report(root,meta,pred,fold_metrics,device):
    ...
    for pi,protocol in enumerate(PROTOCOLS):
        ...
        for family,reference_family in [(f,b) for f in PATH_FAMILIES for b in ["rna","rna_blood"]]+[(f,"Hallmark_score") for f in ["C2_CP_score","C5_BP_score"]]:
        ...
        for group in masks:
            for family in PATH_FAMILIES:
                subgroup_delta.append(add_interval(dict(protocol=protocol,subgroup=group,comparison=family+" minus rna"),model_subgroups[(family,group)]-model_subgroups[("rna",group)]))
    ...
    paired=[]
    for family,ref in [(f,b) for f in PATH_FAMILIES for b in ["rna","rna_blood"]]:
    ...
    labels={"rna":"Hallmark genes","rna_blood":"Hallmark genes + measured cells","Hallmark_score":"Hallmark scores","C2_CP_score":"C2 canonical-pathway scores","C5_BP_score":"C5 BP scores"}
    lines=["# Annotation-defined singscore representations versus retained ridge models", "",
        "Same 528 participants and all original nested folds. Three new ridge procedures use non-directional singscore features from Hallmark, C2 canonical pathways and C5 biological processes. The saved Hallmark gene-level RNA and RNA-plus-measured-cell models are matched comparators. No PD enrichment results are used for feature selection.","",
        "## Primary comparison","",
    ...
    lines += ["","## Demographic reporting","",
        "Subgroup tables use mean repeat-specific pooled out-of-fold metrics, as in Step 1. This differs from the primary within-fold estimand above. Counts and prevalence are in subgroup_counts.tsv; matched pathway-score-minus-RNA changes and conditional intervals are in subgroup_paired_comparisons.tsv.","",
        "## Validation and interpretation","",
        "Saved benchmark predictions and fold assignments have verified provenance; the RNA benchmark agrees with the original mapped-gene run. New score-model fits converged and passed inference checks. Scores agree with an independent base-R implementation of the upstream single-set singscore arithmetic, including min-rank ties and unknown-direction folding. The R singscore package itself was not installed or executed. Held-out perturbation checks protect training backgrounds, score scaling and tuning.","",
        ...
        "Pathways are mapped from complete MSigDB 2026.1.Hs annotations, without cohort-wide significance filtering. Each training partition defines the expressed background and 15–500-member pathway eligibility. Within-sample ranks use original counts divided by fixed featureCounts lengths, which gives the same rank ordering as TPM computed from those counts and lengths. Non-directional scores measure concentration toward expression-rank extremes, not known PD activation. Overlapping gene sets remain correlated. These exploratory comparisons reuse a previously studied cohort; selecting the largest observed score introduces additional selection optimism. No clinical utility, causal pathway effect or independent replication is established.","",
        "![Model comparison](singscore_comparison.png)","",
        "Three final full-cohort score-model artifacts and a raw-count inference script are saved here; these development fits add no performance evidence. Training-specific membership, eligible backgrounds, score parameters and coverage records are retained in folds/.",""]
...
def make_figure(root,primary,repeats):
    ...
    colors=["#555555","#1B8A6B","#247BA0","#E69F00","#CC79A7"]
    for col,protocol in enumerate(PROTOCOLS):
        ...
        for row,metric in enumerate(["AUROC","AP"]):
            ...
            ax.set_xticks(range(5),["Hallmark\ngenes","Genes +\nblood cells","Hallmark\nscores","C2 CP\nscores","C5 BP\nscores"])
            ax.set_xlim(-.4,4.4)
    ...
    fig.suptitle("Pathway-score ridge versus retained Hallmark gene models",fontsize=15,y=.97)
    fig.text(.5,.92,"528 participants • Same nested folds • Three repeats",ha="center",fontsize=10)
    ...
    for ext in ["png","pdf","svg"]:fig.savefig(root/f"singscore_comparison.{ext}",dpi=600,facecolor="white")
    fig.savefig(root/"singscore_comparison_preview.png",dpi=120,facecolor="white")
    plt.close(fig)
