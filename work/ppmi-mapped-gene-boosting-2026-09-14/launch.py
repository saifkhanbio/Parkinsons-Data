...
def prepare():
    ...
    save('environment.json', dict(xgboost=xgboost.__version__, catboost=catboost.__version__, sklearn=sklearn.__version__, numpy=np.__version__, torch=torch.__version__, python=sys.version, executable=sys.executable))
    ...
    paths += [ROOT / n for n in ['README.md', 'estimators.py', 'launch.py', 'predict.py', 'environment.json', 'candidate_genes.tsv', 'fold_plan.json']]
...
def summarize(metrics):
    ...
    assert len(baseline) == 90 and len(current) == 60
...
def train():
    ...
    for number, split in enumerate(plan, 1):
        ...
        for name, rows in [('out_of_fold_predictions.tsv', predictions), ('outer_fold_metrics.tsv', metrics),
            ...
            table(name, rows)
        status('TRAINING', completed_folds=number, total_folds=30, elapsed_seconds=time.monotonic()-started, **context)
    pred = pd.DataFrame(predictions)
    assert len(pred) == 528 * 3 * 2 * 2 and not pred.duplicated(['repeat', 'protocol', 'algorithm', 'PATNO']).any()
    ...
    for ax, protocol in zip(axes, ['participant_stratified', 'batch_grouped']):
        ...
        ax.set(xticks=np.arange(5), xticklabels=algorithms, ylim=(0, 1), title=protocol)
        ax.tick_params(axis='x', labelrotation=45, labelsize=8)
    axes[0].set_ylabel('Mean within-fold AUROC per repeat')
    fig.tight_layout()
    fig.savefig(ROOT / 'model_comparison.png', dpi=600)
    fig.savefig(ROOT / 'model_comparison.pdf')
    plt.close(fig)
    hashes = json.loads((ROOT / 'input_sha256.json').read_text())
    assert all(sha(p) == v for p, v in hashes.items())
    lines = ['# XGBoost and CatBoost mapped-gene comparison', '',
    ...
    lines += ['', 'Primary AUROC averages within-fold AUROCs, then three repeat means. Repeat ranges are not confidence intervals. Paired new-model-minus-baseline differences are in paired_summary.tsv. No AUROC pools margins across models.', '',
        'All count/cohort, nested boundary, matched participant/gene, source-hash, CPU/GPU preprocessing, held-out training-input invariance and serialized inference checks passed. Two final development artifacts are saved.', '',
    ...
    try:
        ...
        with threadpool_limits(limits=2):
            if '--run' in sys.argv:
                ...
                checked = prepare()
                env = os.environ.copy()
                env.update(PYTHONDONTWRITEBYTECODE='1', OMP_NUM_THREADS='2', OPENBLAS_NUM_THREADS='2', MKL_NUM_THREADS='2', MPLCONFIGDIR=str(ROOT / 'matplotlib_cache'))
