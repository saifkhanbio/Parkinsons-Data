...
FROZEN = ROOT / 'approved_cohort_445'
...
def prepare():
    ...
    assert len(m) == 445 and m.PATNO.is_unique and d.PATNO.is_unique
    assert m.group.value_counts().to_dict() == {'PD': 303, 'Control': 142}
    assert m.month_gap_lab_minus_rna.isin([-1, 0]).all()
    ...
    prior = WORK / 'ppmi-blood-cell-adjustment-2026-09-12'
    inputs += [ROOT / 'compare_windows.py', ROOT / 'code_origin.json',
               FROZEN / 'panel_result_provenance.tsv',
               WORK / 'ppmi-blood-counts-2026-09-12/approved_cohort_528/cohort_manifest.tsv',
               prior / 'blood_covariates.tsv', prior / 'hallmark_comparison.tsv']
    for label in MODELS:
        inputs.extend([prior / label / 'results.tsv', prior / label / 'Hallmark_enrichment.tsv'])
...
def finalize():
    ...
    lines = ['# PPMI measured blood-cell adjustment results', '',
             '445 participants: 303 PD and 142 controls. Screening CBCs are from the RNA month or the preceding calendar month.', '',
...
def run():
    try:
        ...
        finalize()
        from compare_windows import compare_windows
        compare_windows()
        ...
        status('COMPLETE', report='TIMING_RESULTS.md', paired_report='RESULTS.md', summary='timing_summary.json')
    except Exception:
